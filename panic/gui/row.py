...
class QAlarmManager(object): #QAlarm):
    ...
    def onContextMenu(self, point):
        ...
        if len([i.acknowledged for i in items]) in (len(items),0):
            self.popMenu.addAction(getThemeIcon("media-playback-pause"), 
                "Acknowledge/Renounce Alarm(s)",self.onAckStateChanged)

        if len([i.disabled for i in items]) in (len(items),0):
            self.popMenu.addAction(getThemeIcon("dialog-error"), 
                "Disable/Enable Alarm(s)",self.onDisStateChanged)
    ...
    def ResetAlarm(self,alarm=None):
        ...
        if alarm is None:
            alarms = [t for t in self.getSelectedAlarms() if t.active]
        else:
            alarms = [alarm]
        msg = 'The following alarms will be reseted:\n\t'+'\n\t'.join(
                [t.tag for t in alarms])
        trace('In AlarmGUI.ResetAlarm(): %s'%msg)
        while len(cmt)==0:
            cmt, ok=prompt.getText(self,'Input dialog',
    ...
    def AcknowledgeAlarm(self,alarm=None):
        ...
        comment, ok = QtGui.QInputDialog.getText(self,
                        'Input dialog','Type a comment to continue:')
